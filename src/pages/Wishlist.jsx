...
export default function Wishlist({ navigate, addToCart, showSuccess }) {
  ...
  const handleAddToCart = (item) => {
    addToCart(item);
    showSuccess(10, "Moved to bag! +10 Points");
  };
        ...
        {wishlist.map(item => (
          <div key={item.id} className="bg-white rounded-3xl p-3 border border-gray-100 shadow-sm relative group">
            <button 
              ...
              className="absolute top-2 right-2 p-1.5 bg-white/80 backdrop-blur-sm rounded-full text-gray-400 hover:text-rose-500 shadow-sm transition-colors z-10"
            >
